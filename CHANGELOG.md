...
- All 5 `react-router-dom` imports (`App.tsx`, `BottomNav.tsx`, `ScrollToTop.tsx`, `Sidebar.tsx`, `Dashboard.tsx`) and the `vite.config.ts` `manualChunks` vendor-react entry now reference `react-router` — same APIs (`BrowserRouter`, `Routes`, `Route`, `Link`, `useLocation`, `useNavigate`), no other code changes needed. All 47 Playwright e2e tests re-verified passing after the swap.
