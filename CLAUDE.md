...
- `src/components/MovieCardGrid.tsx` is the card-grid alternative to `MovieTable.tsx`, toggled via `Segmented` on the Movies page. Column span is `xs={24} sm={12} md={8} lg={6} xl={4}` — 6 cards/row from 1200px up; keep the cover `<img>`'s `width: '100%'` alongside `height` when touching this so `objectFit: 'cover'` has a box to crop against.
...
- `src/utils/poster.ts` exports `POSTER_FALLBACK` (a data-URI placeholder), used wherever a `Movie['Poster URL']` might be blank or fail to load (`MovieCardGrid`, `MovieDrawer`, Dashboard highlight/recent-releases). `Movie['Poster URL']` is optional — always fall back to it rather than conditionally omitting the image element.
